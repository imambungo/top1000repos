<script>
   export let visible_chain_link_index
   export let setVisibleChainLinkIndex
   export let repo
   export let index
   ...
   export let repo_to_highlight

   export let blacklistRepo
   export let removeFromBlackList
   export let sendReport
...
<div id="{repo.full_name}" on:mouseenter={()=>{setVisibleChainLinkIndex(index)}} on:mouseleave={()=>{setVisibleChainLinkIndex(-1)}} on:touchstart={()=>{setVisibleChainLinkIndex(index)}} class="scroll-my-20 flex {repo.full_name == repo_to_highlight ? 'bg-amber-200 transition-colors duration-700' : 'duration-[5000ms]'} {repo.topics.some(topic => excluded_topics.includes(topic)) && 'opacity-50'} -ml-3 md:-ml-2"> <!-- dim if topics is in excluded_topics | https://stackoverflow.com/q/16312528/9157799 | use negative margin left because the space before the number is too big | scroll-my: https://stackoverflow.com/a/60975588/9157799 -->
   <div class="text-sm pt-0.5 w-8 shrink-0 mr-2 text-gray-700 flex flex-col items-end md:ml-0"> <!-- number and link-to-scroll | shrink: https://stackoverflow.com/a/45741742/9157799 -->
      ...
      <div class='grow flex flex-col justify-end'>
         <a class='mb-1 {visible_chain_link_index == index ? 'opacity-100' : 'opacity-0'} hover:opacity-100' href="#{repo.full_name}"> <!-- chain link | https://stackoverflow.com/a/2835151/9157799 | https://github.com/tailwindlabs/tailwindcss/issues/12130 -->
   ...
   <div class='grow flex flex-col gap-1 min-w-0'> <!-- the rest | grow against number | https://stackoverflow.com/a/75308868/9157799 -->
      ...
      <div class='flex flex-wrap gap-x-4 text-xs text-gray-600'> <!-- last_commit_date & PRs thumbs up -->
         ...
         <Top5ClosedPRThumbsUp on:pointerdown={sendReport(`PR: ${repo.full_name}`)} total_thumbs_up_of_top_5_closed_pr_since_1_year={repo.total_thumbs_up_of_top_5_closed_pr_since_1_year} html_url={repo.html_url}/> <!-- pointerdown: https://stackoverflow.com/q/12365339/9157799 -->
         <Top5ClosedIssuesThumbsUp on:pointerdown={sendReport(`ISSUES: ${repo.full_name}`)} total_thumbs_up_of_top_5_closed_issues_since_1_year={repo.total_thumbs_up_of_top_5_closed_issues_since_1_year} html_url={repo.html_url} has_issues_tab={repo.has_issues}/>
