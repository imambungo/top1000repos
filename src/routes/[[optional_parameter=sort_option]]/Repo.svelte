      ...
      <div class='flex gap-2'>
         ...
         <div class='grow flex justify-end items-center'> <!-- browse/hide button -->
            {#if current_tab.tab == 'explore'}
               <button onclick={() => {hidden_repos.add(repo.id)/*; send_report(`hide ${repo.full_name}`)*/}} class="bg-gray-100 hover:bg-gray-200 border text-gray-700 text-xs py-1 px-3 rounded-md"> <!-- https://stackoverflow.com/q/58262380/9157799 -->
                  Hide
               </button>
            {:else if current_tab.tab == 'blacklist'}
               <button onclick={() => {hidden_repos.remove(repo.id)/*; send_report(`remove ${repo.full_name}`)*/}} class="bg-gray-100 hover:bg-gray-200 border text-red-700 text-xs py-1 px-3 rounded-md"> <!-- https://stackoverflow.com/q/58262380/9157799 -->
                  Remove
               </button>
            {/if}
         </div>
